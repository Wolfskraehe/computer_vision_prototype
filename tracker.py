...
class CentroidTracker():
	def __init__(self, maxDisappeared=50):
		...
		# object is allowed to be marked as "disappeared" before deregestering
	...
	def update(self, rects):
		...
		# otherwise, are are currently tracking objects
			...
			for (row, col) in zip(rows, cols):
				# ignored if has been examined before
                ...
                #if a new detection is close enough to the existing one, it is added to it
				if math.dist(self.objects[objectID], inputCentroids[col]) < 70:
					self.objects[objectID] = inputCentroids[col]
					self.disappeared[objectID] = 0
					# keeping track of what has beenn analysed
					usedRows.add(row)
					usedCols.add(col)
                #otherwise assigning a new ID
				else:
			...
			# than the number of existing object centroids register each new input centroid as a trackable object
			else:
